...
menu_key:char;
i,j,n,m,l,k,k0,k1,k2,k_oz,i_oz,j_oz,n_oz,m_oz:integer;//ГЎГ§Г±ГўГ§ВЁВЄВЁ{СЃС‡С‘С‚С‡РёРєРё}
s:string;//temp
lang,lang_f: text;
hf,st:text;
...
'1': begin hero_generate.sex:=1;m:=1;end;//Рњ
'2':begin hero_generate.sex:=2;m:=1;end;//Р–
else
writeln(text[24]);
end;//0.2
end;//0.1
...
writeln(text[26],' ',hero.stren );// СЃРёР»Р°
writeln(text[27],' ',hero.intel );// РёРЅС‚РёР»Р»РµРєС‚
writeln(text[28],' ',hero.agility );// Р»РѕРІРєРѕСЃС‚СЊ
writeln(text[29],' ',hero.sex );// РїРѕР»
writeln(text[30],' ',race_output(hero.race) );// СЂР°СЃСЃР°
writeln(text[31],' ',hero.init );// РёРЅРёС†РёР°С‚РёРІР°
writeln(text[32],' ',hero.masking ); //РјР°СЃРєРёСЂРѕРІРєР°
writeln(text[33],' ',hero.obser );// РЅР°Р±Р»СЋРґР°С‚РµР»СЊРЅРѕСЃС‚СЊ
