...
menu_key:char;
i,j,n,m,l,k,k0,k1,k_oz,i_oz,j_oz,n_oz,m_oz:integer;//Ã¡Ã§Ã±Ã¢Ã§Â¨ÂªÂ¨{ÑÑ‡Ñ‘Ñ‚Ñ‡Ð¸ÐºÐ¸}
s:string;//temp
lang: text;
...
writeln(text[26],' ',hero.stren );// ÑÐ¸Ð»Ð°
writeln(text[27],' ',hero.intel );// Ð¸Ð½Ñ‚Ð¸Ð»Ð»ÐµÐºÑ‚
writeln(text[28],' ',hero.agility );// Ð»Ð¾Ð²ÐºÐ¾ÑÑ‚ÑŒ
writeln(text[29],' ',hero.sex );// Ð¿Ð¾Ð»
writeln(text[30],' ',hero.race );// Ñ€Ð°ÑÑÐ°
writeln(text[31],' ',hero.init );// Ð¸Ð½Ð¸Ñ†Ð¸Ð°Ñ‚Ð¸Ð²Ð°
writeln(text[32],' ',hero.masking ); //Ð¼Ð°ÑÐºÐ¸Ñ€Ð¾Ð²ÐºÐ°
writeln(text[33],' ',hero.obser );// Ð½Ð°Ð±Ð»ÑŽÐ´Ð°Ñ‚ÐµÐ»ÑŒÐ½Ð¾ÑÑ‚ÑŒ
...
case menu_key of
'1': begin 
//Ã¢Â¥Ã¡Ã¢Â®Â¢Â Ã¯ Â£Â¥Â­Â¥Ã Â Ã¦Â¨Ã¯ Â£Â¥Ã Â®Ã¯ Ð³ÐµÐ½ÐµÑ€Ð°Ñ†Ð¸Ñ Ð¿ÐµÑ€ÑÐ¾Ð½Ð°Ð¶Ð° {~ 12.08.2015}
//£¥­¥à æ¨ï ¯¥àá®­ ¦ 
